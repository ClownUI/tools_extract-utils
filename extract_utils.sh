...
function setup_vendor_deps() {
    ...
    export BINARIES_LOCATION="$ANDROID_ROOT"/prebuilts/tools-clown/${HOST}-x86/bin
...
function oat2dex() {
    ...
    if [ -z "$BAKSMALIJAR" ] || [ -z "$SMALIJAR" ]; then
        export BAKSMALIJAR="$ANDROID_ROOT"/prebuilts/tools-clown/common/smali/baksmali.jar
        export SMALIJAR="$ANDROID_ROOT"/prebuilts/tools-clown/common/smali/smali.jar
    fi

    if [ -z "$VDEXEXTRACTOR" ]; then
        export VDEXEXTRACTOR="$ANDROID_ROOT"/prebuilts/tools-clown/${HOST}-x86/bin/vdexExtractor
    fi

    if [ -z "$CDEXCONVERTER" ]; then
        export CDEXCONVERTER="$ANDROID_ROOT"/prebuilts/tools-clown/${HOST}-x86/bin/compact_dex_converter
    fi
